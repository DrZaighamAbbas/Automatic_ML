...
class Auto_Regression(object):
    ...
    def run(self, config, data_frame):
        logging.info("Run Auto_SKlearn Regressoion")
        ...
        print('MSE:')
        print(mean_squared_error(y_test, automl.predict(X_test)))
        print('RMSE:')
        print(np.sqrt(mean_squared_error(y_test, automl.predict(X_test))))
        # print('predict x:')
